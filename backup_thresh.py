...
def dir_thresh(img, sobel_kernel=3, thresh=(0.7, 1.3)):          #0.7이상 1.3 이하이면 255로(이진) ->> 약 40도에서 75도사이
    """
...
def gradient_combine(img, th_x, th_y, th_mag, th_dir):
    ...
    R = img[170:rows - 12, 0:cols, 2]           #BGR : R채널############################################
...
def hls_combine(img, th_h, th_l, th_s):
    ...
    rows, cols = img.shape[:2]
    R = img[170:rows - 12, 0:cols, 2]                                       ############################################
    _, R = cv2.threshold(R, 180, 255, cv2.THRESH_BINARY)
    #cv2.imshow('red!!!',R)
    H = hls[170:rows - 12, 0:cols, 0]                                       ############################################
    L = hls[170:rows - 12, 0:cols, 1]                                       ############################################
    S = hls[170:rows - 12, 0:cols, 2]                                       ############################################
    ...
    #hls_comb[((s_img > 1) & (l_img == 0)) | (R > 1)] = 255     #((s_img == 0) & (h_img > 1) & (l_img > 1))빼버림 ?????
...
def comb_result(grad, hls):
    ...
    result[(grad > 1)] = 100                                #numpy연산이라서 가능
    result[(hls > 1)] = 255
